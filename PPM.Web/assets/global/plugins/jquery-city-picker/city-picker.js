...
(function (factory) {
    ...
    CityPicker.prototype = {
        ...
        refresh: function (force) {
            ...
            //this.feedVal();
            //this.feedText();
            var titleSpan = this.$textspan.find("span[class=title]");
            if (this.$element.val()) {
                this.$textspan.find("span[class=placeholder]").remove();
                titleSpan.html("<span>" + this.$element.val() + "</span>");
            }
        ...
        feedVal: function (trigger) {
            ...
            if(trigger) {
                ...
                var areaName = this.$element.attr("data-area-name");
                var areaElement = $("input[name='" + areaName + "']");
                if (areaElement) {
                    //HARD CODE
                    //澳门
                    if (this.getCode("province") == "820000") {
                        areaElement.val(this.getCode("city")).trigger('change');
                    }
                    //香港
                    else if (this.getCode("province") == "810000") {
                        areaElement.val(this.getCode("district")).trigger('change');
                    }
                    //台湾
                    else if (this.getCode("province") == "710000") {
                        areaElement.val(this.getCode("district")).trigger('change');
                    } else {
                        areaElement.val(this.getCode("town")).trigger('change');    
                    }
                } else {
                    areaElement.val("").trigger('change');
                }
            }
        },
        ...
        output: function (type) {
            ...
            $.get('/Common/GetAreasByParentAreaId?parentAreaId=' + code, function (result) {
                if (result) {
                    //var provs = [];
                    $.each(result, function (code, address) {
                        data.push({
                            code: address.code,
                            address: address.address,
                            selected: address.address === value
                        });
                        //if (type === PROVINCE) {
